...
const translations = {
    ...
    "bookshelf.fiction": "Художественная и Научпоп",
    ...
    "bookshelf.personal.4": '"Как завоёвывать друзей и ..." — Дейл Карнеги',
    "bookshelf.personal.5": '"45 татуировок продавана" — Максим Батырев',
    "bookshelf.personal.6": '"Богатый папа, бедный папа" — Роберт Кийосаки',
...
function updateText(language) {
    if (language === 'en') {
        // Восстанавливаем оригинальный текст из HTML
    ...
    // Для русского языка подставляем переводы
    document.querySelectorAll('[data-key]').forEach(item => {
        const key = item.getAttribute('data-key');
